...
public class ParsePushPlugin extends CordovaPlugin {
    ...
    private static final String ACTION_FLUSH_QUEUE = "actionFlushQueue";
...
//            if (!pnQueue.isEmpty()) {
//                Log.d("ParsePushPluginQueue", String.valueOf(pnQueue.size()));
//                flushPNQueue();
//            }
            return true;
        }

        if (action.equals(ACTION_FLUSH_QUEUE)) {
            if (!pnQueue.isEmpty()) {
                Log.d("ParsePushPluginQueue", String.valueOf(pnQueue.size()));
                flushPNQueue();
            }
    ...
    private static void flushPNQueue() {
        while (!pnQueue.isEmpty() && gEventCallback != null) {
            Log.d("ParsePushPluginQueue", String.valueOf(pnQueue));
